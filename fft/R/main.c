...
int main (int argc, char**argv) 
{
   ...
   size_t length; 
   double* re;
   int rc = singen (table_size, sps, freq, phase, duration_sec, amp, &length, &re);
   if (rc) 
   {
      exit(1);
   }
   ...
   */
   ...
   int inv = -2;
   double* im = (double*)calloc (length, sizeof (double));
   rc2 = fft_work(re, im, 1, length, 1, inv, work, iwork);
