...
function drawAxis() {
    ...
    for (i = 0; i < total_minor_grids_y + 2; ++i) {
        var y = xgrid_y0 + minor_grid_step_y * i;
        if (((y - center_y) % (minor_grid_per_major * minor_grid_step_y)) == 0) {
            ctx.beginPath();
        ...
        drawLine(graph_x0,  y + 0.5,
                graph_x0 + graph_w, y + 0.5);
    }
    ctx.strokeStyle = minorStyle;
    ctx.lineWidth = '1';
    ctx.beginPath();
    ...
    for (i = 0; i < total_minor_grids_x + 2; ++i) {
        ...
        var x = ygrid_x0 + minor_grid_step_x * i;
        if (((x - center_x) % (minor_grid_per_major * minor_grid_step_x)) == 0) {
            ctx.beginPath();
            ctx.strokeStyle = majorStyle;
        }
        else {
        ...
        drawLine(x + 0.5, graph_y0, x + 0.5, graph_y0 + graph_h);
    }
